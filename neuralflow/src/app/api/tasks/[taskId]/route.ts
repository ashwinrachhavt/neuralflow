...
export async function PATCH(req: Request, { params }: RouteContext) {
  ...
  const task = await getByIdForUser(params.taskId, (user as any).id).catch(() => null);
  if (!task) {
    return NextResponse.json({ message: "Not found" }, { status: 404 });
  }

  const updated = await updateTitle(params.taskId, title, (user as any).id);
...
export async function DELETE(_req: Request, { params }: RouteContext) {
  ...
  await deleteForUser(params.taskId, (user as any).id);
