...
type ForceGraphFactory = (element: HTMLElement) => any;

export default function EmbeddingsGraph() {
  const [nodes, setNodes] = useState<Node[]>([]);
  const containerRef = useRef<HTMLDivElement | null>(null);
  const [ready, setReady] = useState(false);
  const graphFactoryRef = useRef<ForceGraphFactory | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (cancelled) return;
      const factory = (mod.default ?? mod) as unknown as ForceGraphFactory;
      graphFactoryRef.current = factory;
      setReady(true);
    })();
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    if (!ready || !containerRef.current || !graphFactoryRef.current) return;
    const graph: any = graphFactoryRef.current(containerRef.current)
      .graphData({
